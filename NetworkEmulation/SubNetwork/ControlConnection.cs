   ...
   public class MessageCC
    ...
    public class ControlConnection
    {
        ...
        public ControlConnection(string numberCC)
        {
            ...
            listOfTrace = new List<List<string>>();
        ...
        private void chooiceAction(string[] message)
        {
            ...
            else if (action == MessageNames.ROUTE_TABLE_QUERY)
            {
                ...
                frequency = message[2];
                    ...
                    listOfLink.Add(message[4]);
                    listOfLink.Add(returnString(message));
                    for(int i=5; i<message.Length-1; i+=2)
                    {
                        listOfLink.Add(message[i]);
                    }
                    listOfLink.Add(connectionBitrate);
                    listOfLink.Add(frequency);
                    listOfLink.Add(numberOfHops);
                    listOfLink.Add(message[message.Length-2]);
                    listOfTrace.Add(listOfLink);
            ...
            else
            {
                string messageNotConfirmed = ConfigurationManager.AppSettings["CC" + numberCC] + "#" + MessageNames.CONNECTION_TEARDOWN_CONFIRMED + "NOT" +
                    ...
                    if (response.ip != "")
                    {
                        ...
                        Console.WriteLine("[" + Timestamp.generateTimestampCC() + "]" + " It is could not to establish connection from " + transponderStart.addressStart + " to " + transponderStart.addressEnd);
                        Console.ResetColor();
                    }
            }
            ...
            else if(action==MessageNames.KILL_LINK)
            {
                killLinkHandling(message);
            }
        }

        private void killLinkHandling(string [] message)
        {
            generateLogReceived("LRM", MessageNames.KILL_LINK, ConfigurationManager.AppSettings["LRM" + numberCC]);

            int index = listOfTrace.Count;
            string startAddress="";
            string endAddress="";

            response = new MessageStruct();
            response.ip = "";
            response.function = MessageNames.CONNECTION_TEARDOWN + "NCC";

            if(numberCC=="1" || numberCC=="5")
            {
                mainCC = true;
            ...
            index = searchInList(message[2]);


            if (index != listOfTrace.Count)
            {

                startAddress = listOfTrace[index][0];
                endAddress = listOfTrace[index][listOfTrace[index].Count - 1];
                connectionBitrate = listOfTrace[index][listOfTrace[index].Count - 4];
                frequency = listOfTrace[index][listOfTrace[index].Count - 3];
                numberOfHops = listOfTrace[index][listOfTrace[index].Count - 2];
                ...
                getPathHandling(listOfTrace[index][1].Split('#'));
        ...
        private int searchInList(string key)
        {
            int index;

            for (int i = 0; i < listOfTrace.Count; i++)
            {
                for (int j = 0; j < listOfTrace[i].Count-4; j++)
                {
                    if ((listOfTrace[i])[j] == key)
                    {
                        index = i;
                        return index;
                    }
            ...
            return listOfTrace.Count;
        }

        private int searchInList(string addressStart,string addressEnd)
        {
            int index;
            ...
            for (int i = 0; i < listOfTrace.Count; i++)
            {
                    ...
                    if ((listOfTrace[i])[0] == addressStart && (listOfTrace[i])[(listOfTrace[i]).Count-1] == addressEnd)
                    {
                        index = i;
                        return index;
                    }
        ...
        private string returnString (string[] message)
        {
            string longString="";
            for(int i=0; i<message.Length-1; i++)
            {
                longString += (message[i] + "#");
            }
            return longString;
        }
        ...
        private void connectionTeardownConfirmedHandling(string []message)
        {
            ...
            if (listOfMessages.Count == 0)
            {
                ...
                if (response.ip != "")
                {
                    generateLogSend("NCC", MessageNames.CONNECTION_TEARDOWN_CONFIRMED, response.ip);
                    SendingMessage(response.ip, responseMessageToUp);

                    int index2 = searchInList(transponderStart.addressStart, transponderStart.addressEnd);
                    connectionBitrate = listOfTrace[index2][listOfTrace[index2].Count - 4];
                    numberOfHops = listOfTrace[index2][listOfTrace[index2].Count - 2];
                    frequency = listOfTrace[index2][listOfTrace[index2].Count - 3];

                    listOfTrace.RemoveAt(index2);
                }
                else
                {
                    ...
                    response.function = MessageNames.CONNECTION_REQUEST;

                    string messageToSend = ConfigurationManager.AppSettings["CC" + numberCC] + "#" + MessageNames.ROUTE_TABLE_QUERY +
                    "#" + transponderStart.addressStart + "#" + transponderStart.addressEnd + "#" + connectionBitrate + "#" +numberOfHops + "#";
                    ...
                    else
                    {
                        ...
                        if (mainCC == true)
                        {
                            ...
                            if (Int32.Parse(numberOfHops) == 1)
                            {
                                ...
                                borderTableFillingMessage(transponderStart, MessageNames.CONNECTION_TEARDOWN, ConfigurationManager.AppSettings[messagesCC[0].addressStart]);
                                ...
                                //Uzupelnienie tablicy border node commutation table
                                borderTableFillingMessage(transponderEnd, MessageNames.CONNECTION_TEARDOWN, ConfigurationManager.AppSettings[messagesCC[messagesCC.Count - 1].addressEnd]);
        ...
        public void getPathHandling(string[] message)
        {
            ...
            else
            {
                ...
                SendingMessage(response.ip, messageNotConfirmed);
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine("["+ Timestamp.generateTimestampCC()  + "]" + " It is could not to break connection from " + transponderStart.addressStart + " to " + transponderStart.addressEnd);
                Console.ResetColor();
        ...
        public void ConnectionDeleteHandling(string[] message)
        {
            ...
            }else if(typeObject=="BORDERTABLE")
            {
                string startAddress = message[4];
                string port = message[5];
                string endAddress = message[6];
                frequency = message[7];
                //message[8] przekazany adress CC docelowego routera

                transponderStart = new MessageCC("", startAddress, "", endAddress, "");
                transponderStart.addressCC = message[8];
                transponderStart.startPort = port;
                generateLogSend("CC", MessageNames.CONNECTION_REQUEST, message[8]);
                borderTableFillingMessage(transponderStart, MessageNames.CONNECTION_TEARDOWN, "");
                generateLogReceived("CC", MessageNames.CONNECTION_TEARDOWN, message[8]);
            }
        }
        ...
        private void ConnectionRequestHandling(string[] message)
        {
            ...
            if (typeObject.StartsWith("NCC"))
            {
                ...
                if (code == "PUT")
                {
                    ...
                    string messageToSend = ConfigurationManager.AppSettings["CC" + numberCC] + "#"+ MessageNames.ROUTE_TABLE_QUERY +
                    "#" + startAddress + "#" + endAddress + "#" + bitrate + "#" + hops + "#";
                    generateLogSendRC("RC",MessageNames.ROUTE_TABLE_QUERY, ConfigurationManager.AppSettings["RC" + numberCC],bitrate);
                    SendingMessage(ConfigurationManager.AppSettings["RC" + numberCC], messageToSend);
                }
            }
            else if (typeObject=="CC")
            ...
            }else if(typeObject=="BORDERTABLE")
            {
                ...
                generateLogReceived("CC", MessageNames.CONNECTION_REQUEST, message[8]);
            }
        }
                    ...
                    else
                    {
                        ...
                        if (mainCC == true)
                        {
                            ...
                            if (Int32.Parse(numberOfHops) == 1)
                            {
                                ...
                                //Uzupelnienie tablicy border node commutation table
                                borderTableFillingMessage(transponderStart, MessageNames.CONNECTION_REQUEST, ConfigurationManager.AppSettings[messagesCC[0].addressStart]);

                                transponderEnd.addressCC = messagesCC[messagesCC.Count - 1].addressCC;
                                transponderEnd.startPort = messagesCC[messagesCC.Count - 1].endPort;

                                //Uzupelnienie tablicy border node commutation table
                                borderTableFillingMessage(transponderEnd, MessageNames.CONNECTION_REQUEST, ConfigurationManager.AppSettings[messagesCC[messagesCC.Count - 1].addressEnd]);
        ...
        public void borderTableFillingMessage(MessageCC transponder, string operation,string addressIPCC)
        {
            ...
            string MessageToCC = ConfigurationManager.AppSettings["CC" + numberCC] + "#" + operation + "#" +
                                 ...
                                 "#" + transponder.addressEnd + "#" + frequency + "#"+addressIPCC+"#";
        ...
        public static void generateLogReceived(string nameRemoteObject, string function,string address)
        {
            ...
            Console.ForegroundColor =  ConsoleColor.DarkRed;
            Console.WriteLine("[" + Timestamp.generateTimestampCC()  + "]" + " CC received message " + function + " from " +
        ...
        {
            Console.ForegroundColor = ConsoleColor.DarkRed;
            Console.WriteLine("[" + Timestamp.generateTimestampCC()  + "]" + " CC send message " + function + " to " +
                              nameRemoteObject+" IP:"+address);
            Console.ForegroundColor = ConsoleColor.Black;

        }

        public static void generateLogSendRC(string nameRemoteObject, string function, string address,string bitrate)
        {
            Console.ForegroundColor = ConsoleColor.DarkRed;
            Console.WriteLine("[" + Timestamp.generateTimestampCC() + "]" + " CC send message " + function + " to " +
                              nameRemoteObject + " IP:" + address +" to estabilsh connection with bitrate: " +bitrate);

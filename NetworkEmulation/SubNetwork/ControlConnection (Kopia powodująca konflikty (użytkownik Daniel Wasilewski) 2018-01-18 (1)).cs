    ...
    public class ControlConnection
    {
        ...
        public void ConnectionDeleteHandling(string[] message)
        {
            ...
            if (typeObject.StartsWith("NCC"))
            {
                ...
                if (code == "PUT")
                {
                    generateLogReceived(typeObject, MessageNames.CONNECTION_TEARDOWN);
                    response = new MessageStruct();
                    response.ip = message[0];
                    response.function = message[1];
                    ...
                    "#" + startAddress + "#" + endAddress + "#" + hops + "#" + frequency + "#";
                    SendingMessage(ConfigurationManager.AppSettings["RC" + numberCC], messageToSend);
                }
            }
            ...
            else
            {
                ...
                {
                    generateLogReceived(typeObject, MessageNames.CONNECTION_TEARDOWN_CONFIRMED);
                            ...
                            if (mainCC == true)
                            {
                                if (numberOfHops == "1")
                                {
                                    string responseMessageToUp =
                                        ...
                                        MessageNames.CONNECTION_TEARDOWN_CONFIRMED + "#" + "OK" + "#";
                                ...
                                {
                                    if (response.function == MessageNames.CONNECTION_TEARDOWN)
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        generateLogSend("CC", MessageNames.CONNECTION_TEARDOWN + "RESPONSE");
                                    }
                                }
                            }
        ...
        private void ConnectionRequestHandling(string[] message)
        {
            ...
            else
            {
                ...
                {
                    generateLogReceived("CC", MessageNames.CONNECTION_REQUEST + "RESPONSE");
                    string[] tmp = message;
                    int index = listOfMessages.FindIndex(x => x.function == MessageNames.CONNECTION_REQUEST);
                    MessageStruct msg = listOfMessages.ElementAt(index);
                    msg.count = msg.count - 1;
